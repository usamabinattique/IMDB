...
public struct Endpoint<A> {
    ...
    public func map<B>(_ f: @escaping (A) -> B) -> Endpoint<B> {
        Endpoint<B>(request: request, expectedStatusCode: expectedStatusCode, parse: { value, response in
...
public protocol NetworkLoader {
    func load<A>(_ e: Endpoint<A>, onComplete: @escaping (Result<A, Error>) -> ()) -> URLSessionDataTaskProtocol
}
...
extension URLSession: NetworkLoader {
  ...
  public func load<A>(_ e: Endpoint<A>, onComplete: @escaping (Result<A, Error>) -> ()) -> URLSessionDataTaskProtocol {
    let r = e.request
...
extension URLSession {
    ...
    public func load<A, S:Scheduler>(_ e: Endpoint<A>, on scheduler: S) -> AnyPublisher<A, Error> {
        let r = e.request
